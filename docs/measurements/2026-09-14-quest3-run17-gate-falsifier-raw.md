...
цена разогрева PSO, мс:         5.074 (пик N=100 фазы C–D; компиляцией не подтверждён — см. W)
старт до _ready, мс:            1476 (baker выкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 6.629 мс
разброс прогонов, мс:           0.002
...
ожидается исполненных проверок: 55
...
бюджет кадра: 11.11 мс при 90.0 Гц — ВСЕ числа ниже относятся к этой частоте
  PASS  частота: запрошена цель 90.0 Гц — получено 90.0 (было 72.0), ждали 88 мс
--- A. Возможности ---
устройство:   Adreno (TM) 740
вендор:       Qualcomm
Vulkan API:   1.3.295
...
сборка: shader baker выкл (res://.godot/shader_cache в пакете)
  PASS  W старт: 1476 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker выкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 6.779 / GPU 3.302 против установившихся 0.150 / 3.240 мс — цена 6.629 мс
  FAIL  W контроль повтора: пик 4.857 мс против 6.629 в первый раз — кэш не снял цену, пик первого эпизода НЕ подтверждён как компиляция; счётчики { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 }
...
!!! ФАЛЬСИФИКАТОР ГЕЙТА: точка N=400 получит 300 объектов — гейт обязан покраснеть, числа свипа недействительны !!!

свип «небатченый»:
    N=0     draw calls движка=0      CPU n=135  сред=0.129  мед=0.120  p95=0.174  макс=0.206 мс
                          GPU n=135  сред=2.871  мед=2.865  p95=2.984  макс=3.119 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=50    draw calls движка=50     CPU n=135  сред=0.205  мед=0.195  p95=0.250  макс=0.255 мс
                          GPU n=135  сред=3.023  мед=3.020  p95=3.115  макс=3.291 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=100   draw calls движка=100    CPU n=120  сред=0.220  мед=0.207  p95=0.306  макс=0.400 мс
                          GPU n=120  сред=4.205  мед=4.305  p95=5.136  макс=15.918 мс
                          доставлено 120 кадров за 1503 мс = 79.8 к/с из 90 (89%); сверх бюджета 1 (0.8%), самая длинная серия 1 подряд
    N=200   draw calls движка=200    CPU n=136  сред=0.319  мед=0.316  p95=0.415  макс=0.736 мс
                          GPU n=136  сред=5.521  мед=5.207  p95=7.767  макс=8.427 мс
                          доставлено 136 кадров за 1504 мс = 90.4 к/с из 90 (100%); промахов по бюджету нет
    N=400   draw calls движка=300    CPU n=135  сред=0.497  мед=0.491  p95=0.679  макс=0.895 мс
                          GPU n=135  сред=4.427  мед=4.419  p95=5.662  макс=5.936 мс
                          доставлено 135 кадров за 1504 мс = 89.8 к/с из 90 (100%); промахов по бюджету нет
    N=800   draw calls движка=800    CPU n=135  сред=0.893  мед=0.846  p95=1.193  макс=1.660 мс
                          GPU n=135  сред=6.617  мед=6.555  p95=6.851  макс=6.986 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=1600  draw calls движка=1600   CPU n=135  сред=1.999  мед=1.979  p95=2.169  макс=4.458 мс
                          GPU n=135  сред=9.562  мед=9.527  p95=9.842  макс=9.936 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
свип «инстансы»:
    N=0     draw calls движка=0      CPU n=135  сред=0.113  мед=0.107  p95=0.152  макс=0.186 мс
                          GPU n=135  сред=2.645  мед=2.620  p95=2.871  макс=3.170 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=50    draw calls движка=1      CPU n=135  сред=0.142  мед=0.135  p95=0.166  макс=0.446 мс
                          GPU n=135  сред=2.942  мед=2.856  p95=3.374  макс=3.600 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=100   draw calls движка=1      CPU n=135  сред=0.141  мед=0.136  p95=0.163  макс=0.213 мс
                          GPU n=135  сред=2.935  мед=2.849  p95=3.296  макс=3.560 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=200   draw calls движка=1      CPU n=135  сред=0.142  мед=0.136  p95=0.166  макс=0.283 мс
                          GPU n=135  сред=2.962  мед=2.873  p95=3.297  макс=3.641 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=400   draw calls движка=1      CPU n=135  сред=0.154  мед=0.136  p95=0.183  макс=0.888 мс
                          GPU n=135  сред=3.027  мед=2.924  p95=3.609  макс=4.703 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=800   draw calls движка=1      CPU n=135  сред=0.141  мед=0.137  p95=0.164  макс=0.220 мс
                          GPU n=135  сред=2.967  мед=2.899  p95=3.393  макс=3.529 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=1600  draw calls движка=1      CPU n=135  сред=0.138  мед=0.135  p95=0.159  макс=0.202 мс
                          GPU n=135  сред=2.945  мед=2.890  p95=3.291  макс=3.564 мс
                          доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет

разброс двух прогонов на N=100: 0.002 мс (0.243 против 0.245)
  PASS  разрешение свипа: разброс 0.002 мс меньше шага 1.859 мс (в 929 раз) — наклону можно верить
  PASS  нулевой контроль: пустая сцена CPU 0.120 / GPU 2.865 мс, связывает GPU — 26% бюджета 11.11 мс, запас есть
  FAIL  гейт «небатченый»: точки с расхождением заявленного и счётчика движка: N=400→300 — СВИП НЕВАЛИДЕН
  ...
  PASS  разогрев PSO: пик первых 73 кадров выше установившегося на 5.074 мс (макс 5.281 против мед 0.207)
  ????  наклон «небатченый»: не считаю — свип аннулирован (небатченый)
  ????  наклон «инстансы»: не считаю — свип аннулирован (небатченый)
...
passed=45 failed=4 unknown=6
пол: исполнено 55/55 проверок
```
