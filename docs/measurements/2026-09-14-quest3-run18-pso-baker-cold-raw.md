...
старт до _ready, мс:            2899 (baker вкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 889.073 мс
...
бюджет кадра: 11.11 мс при 90.0 Гц — ВСЕ числа ниже относятся к этой частоте
  PASS  частота: запрошена цель 90.0 Гц — получено 90.0 (было 72.0), ждали 48 мс
--- A. Возможности ---
устройство:   Adreno (TM) 740
вендор:       Qualcomm
Vulkan API:   1.3.295
...
сборка: shader baker ВКЛ (res://.godot/shader_cache в пакете)
  PASS  W старт: 2899 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker вкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 889.226 / GPU 4.363 против установившихся 0.153 / 3.465 мс — цена 889.073 мс
  PASS  W контроль повтора: пик 5.581 мс против 889.073 в первый раз (в 159 раз меньше) — первый пик был компиляцией, кэш её снял; счётчики повтора { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } — это запросы, а не компиляции с нуля
