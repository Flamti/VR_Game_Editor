...
старт до _ready, мс:            1437 (baker вкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 5.550 мс
...
сборка: shader baker ВКЛ (res://.godot/shader_cache в пакете)
  PASS  W старт: 1437 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker вкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 5.703 / GPU 4.931 против установившихся 0.153 / 3.266 мс — цена 5.550 мс
  FAIL  W контроль повтора: пик 5.581 мс против 5.550 в первый раз — кэш не снял цену, пик первого эпизода НЕ подтверждён как компиляция; счётчики { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 }
