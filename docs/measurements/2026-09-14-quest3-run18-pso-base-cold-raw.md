...
старт до _ready, мс:            3119 (baker выкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 918.728 мс
...
бюджет кадра: 11.11 мс при 90.0 Гц — ВСЕ числа ниже относятся к этой частоте
  PASS  частота: запрошена цель 90.0 Гц — получено 90.0 (было 72.0), ждали 40 мс
--- A. Возможности ---
устройство:   Adreno (TM) 740
вендор:       Qualcomm
Vulkan API:   1.3.295
...
сборка: shader baker выкл (res://.godot/shader_cache в пакете)
  PASS  W старт: 3119 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker выкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 918.875 / GPU 3.406 против установившихся 0.147 / 3.200 мс — цена 918.728 мс
  PASS  W контроль повтора: пик 5.505 мс против 918.728 в первый раз (в 167 раз меньше) — первый пик был компиляцией, кэш её снял; счётчики повтора { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } — это запросы, а не компиляции с нуля
