...
старт до _ready, мс:            2815 (baker вкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 892.376 мс
...
бюджет кадра: 11.11 мс при 90.0 Гц — ВСЕ числа ниже относятся к этой частоте
  PASS  частота: запрошена цель 90.0 Гц — получено 90.0 (было 72.0), ждали 445 мс
--- A. Возможности ---
устройство:   Adreno (TM) 740
вендор:       Qualcomm
Vulkan API:   1.3.295
...
сборка: shader baker ВКЛ (res://.godot/shader_cache в пакете)
  PASS  W старт: 2815 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker вкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 892.522 / GPU 4.152 против установившихся 0.146 / 3.231 мс — цена 892.376 мс
  PASS  W контроль повтора: пик 5.028 мс против 892.376 в первый раз (в 177 раз меньше) — первый пик был компиляцией, кэш её снял; счётчики повтора { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } — это запросы, а не компиляции с нуля
  PASS  W2 предзагрузка СНИМАЕТ пик: A (.tres на поверхности, предзагружен): пик 4.729 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 0, "draw": 1, "specialization": 1 }; B (override из .tres, без предзагрузки): пик 1101.129 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 4, "draw": 1, "specialization": 1 }; baker вкл
