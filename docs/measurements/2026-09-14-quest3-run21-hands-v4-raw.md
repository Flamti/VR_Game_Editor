...
старт до _ready, мс:            1735 (baker выкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 3.776 мс
...
ожидается исполненных проверок: 69
...
бюджет кадра: 11.11 мс при 90.0 Гц — ВСЕ числа ниже относятся к этой частоте
  PASS  частота: запрошена цель 90.0 Гц — получено 90.0 (было 72.0), ждали 50 мс
--- A. Возможности ---
устройство:   Adreno (TM) 740
вендор:       Qualcomm
Vulkan API:   1.3.295
...
сборка: shader baker выкл (res://.godot/shader_cache в пакете)
  PASS  W старт: 1735 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker выкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 3.928 / GPU 4.916 против установившихся 0.152 / 3.285 мс — цена 3.776 мс
  FAIL  W контроль повтора: пик 5.694 мс против 3.776 в первый раз — кэш не снял цену, пик первого эпизода НЕ подтверждён как компиляция; счётчики { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 }
  ????  W2 предзагрузка: контроль B без пика компиляции — кэш тёплый, сравнивать нечего. A (.tres на поверхности, предзагружен): пик 5.113 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 0, "draw": 1, "specialization": 1 }; B (override из .tres, без предзагрузки): пик 6.012 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 4, "draw": 1, "specialization": 1 }; baker выкл

--- R v4. Руки: гейт контекста щипка, запас, кулак при вращении (интерактивная, ADR-0008) ---
пороги: кулак вход 0.40 / выход 0.24 по min-сгибу; щипок вход 0.0165 / выход 0.030 м при не-кулаке; отпускание 35 мс; гейт щипка: сгиб среднего и безымянного < 0.063
  срабатывания классификатора по окнам (рука:класс@секунда):
    rest     трекинг L 100% R 100%  нет
    wrist_L  трекинг L 100% R 100%  нет
    palm_L   трекинг L 100% R 100%  нет
    palm_R   трекинг L 100% R 100%  нет
    pinch_R1 трекинг L 100% R 100%  R:pinch@1.0 R:pinch@3.5 R:pinch@6.5
    pinch_L1 трекинг L 100% R 100%  L:pinch@0.8 L:pinch@3.4 L:pinch@6.5
    pinch_R2 трекинг L 100% R 100%  R:pinch@0.8 R:pinch@3.6 R:pinch@6.7
    pinch_L2 трекинг L 100% R 100%  L:pinch@0.6 L:pinch@3.6 L:pinch@7.1
    pinch_R3 трекинг L 100% R 100%  R:pinch@0.7 R:pinch@3.5 R:pinch@6.5
    pinch_L3 трекинг L 100% R 100%  L:pinch@0.4 L:pinch@3.5 L:pinch@6.4
    fist_L   трекинг L 100% R 100%  L:fist@0.7 L:fist@3.6 L:fist@6.6 L:fist@9.6
    poke_R   трекинг L 100% R 100%  нет
    rest_relaxed трекинг L 100% R 100%  нет
    pinch_relax_R трекинг L 100% R 100%  R:pinch@0.7 R:pinch@3.7 R:pinch@6.7 R:pinch@9.4
    pinch_slow_R трекинг L 100% R 100%  R:pinch@1.5 R:pinch@5.9 R:pinch@11.0
    rot_fist_L трекинг L 100% R 100%  L:fist@1.1 L:fist@4.8 L:fist@8.7 L:fist@12.0
  касаний шарика 3; строк TSV 33090; сетка руки { "L": "ready", "R": "ready" }
  PASS  R контроль: обе руки ведёт профиль /interaction_profiles/ext/hand_interaction_ext
  PASS  R трекинг L: все 26 суставов в 100% кадров окна покоя
  PASS  R трекинг R: все 26 суставов в 100% кадров окна покоя
  PASS  R суставы правой: медиана 26 из 26
  PASS  R фальсификатор 1 (покой): классификатор молчит
  PASS  R фальсификатор 2 (вращение левой кистью): классификатор молчит
  PASS  R ладонь L: разделяет palm_head — palm_head: к лицу +0.99, от лица -0.94; palm_y_head: к лицу -0.02, от лица +0.10
  PASS  R ладонь R: разделяет palm_head — palm_head: к лицу +0.99, от лица -0.97; palm_y_head: к лицу -0.13, от лица -0.02
  PASS  R ладонь L против свидетеля: menu_gesture горит в 100% кадров palm_head > +0.5 (489) и в 1% кадров < −0.5 (416) — признак согласен с рантаймом
  PASS  R pinch R по сигналу: 9 из 9 сигналов, лишних нет
  PASS  R pinch L по сигналу: 9 из 9 сигналов, лишних нет
  PASS  R fist L по сигналу: 4 из 4 сигналов, лишних нет
  PASS  R касания шарика: ровно 3
  PASS  R перекрёстные срабатывания: нет
  ????  R фальсификатор 3 (расслабленные руки): молчат оба классификатора — окно не задело гейт, свободного щипка не было
  PASS  R эффект гейта: снято poke_R/L −2; в окнах щипка по сигналу не снято ни одного
  PASS  R pinch R по сигналу (relax): 4 из 4 сигналов, лишних нет
  медленный щипок: минимум расстояния по слотам cue1 6.0 мм, cue2 4.9 мм, cue3 5.1 мм; порог входа 16.5 мм, запас худшего слота 10.5 мм
  PASS  R pinch R по сигналу (slow): 3 из 3 сигналов, лишних нет
  PASS  R fist L по сигналу (rot): 4 из 4 сигналов, лишних нет
  PASS  R лог признаков: 33090 строк
  PASS  R сетка руки: модель обеих рук от рантайма
...
passed=60 failed=3 unknown=6
пол: исполнено 69/69 проверок
```
