...
старт до _ready, мс:            1487 (baker выкл)
W: компиляций DRAW новый/повтор: 1 / 1, пик 3.992 мс
...
сборка: shader baker выкл (res://.godot/shader_cache в пакете)
  PASS  W старт: 1487 мс от запуска движка до _ready; компиляций к этому моменту { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }; baker выкл
  PASS  W новый вариант: компиляции за 30 кадров { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 } (позже ещё { "canvas": 0, "mesh": 0, "surface": 0, "draw": 0, "specialization": 0 }); пик CPU 4.142 / GPU 5.265 против установившихся 0.150 / 3.265 мс — цена 3.992 мс
  FAIL  W контроль повтора: пик 5.971 мс против 3.992 в первый раз — кэш не снял цену, пик первого эпизода НЕ подтверждён как компиляция; счётчики { "canvas": 0, "mesh": 0, "surface": 2, "draw": 1, "specialization": 1 }
  ????  W2 предзагрузка: контроль B без пика компиляции — кэш тёплый, сравнивать нечего. A (.tres на поверхности, предзагружен): пик 4.783 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 0, "draw": 1, "specialization": 1 }; B (override из .tres, без предзагрузки): пик 6.458 мс, компиляции { "canvas": 0, "mesh": 0, "surface": 4, "draw": 1, "specialization": 1 }; baker выкл
...
частота 90.0 Гц, бюджет 11.11 мс; режимы ["выкл", "вкл", "вкл+relax"] в порядке ["выкл", "вкл", "вкл+relax", "вкл+relax", "вкл", "выкл"]; точки [0, 1600]
  ...
  режим выкл (повтор 1):
    N=0     отрисовок 0     GPU 3.214  CPU 0.129 мс; доставлено 136 кадров за 1510 мс = 90.1 к/с из 90 (100%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 8.949  CPU 1.869 мс; доставлено 135 кадров за 1501 мс = 89.9 к/с из 90 (100%); промахов по бюджету нет
  режим вкл (повтор 1):
    N=0     отрисовок 0     GPU 2.807  CPU 0.117 мс; доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 13.061  CPU 2.318 мс; доставлено 115 кадров за 1505 мс = 76.4 к/с из 90 (85%); сверх бюджета 115 (100.0%), самая длинная серия 115 подряд
  режим вкл+relax (повтор 1):
    N=0     отрисовок 0     GPU 3.640  CPU 0.236 мс; доставлено 68 кадров за 1510 мс = 45.0 к/с из 90 (50%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 17.655  CPU 3.087 мс; доставлено 68 кадров за 1510 мс = 45.0 к/с из 90 (50%); сверх бюджета 68 (100.0%), самая длинная серия 68 подряд
  режим вкл+relax (повтор 2):
    N=0     отрисовок 0     GPU 3.641  CPU 0.211 мс; доставлено 68 кадров за 1511 мс = 45.0 к/с из 90 (50%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 16.371  CPU 2.787 мс; доставлено 68 кадров за 1511 мс = 45.0 к/с из 90 (50%); сверх бюджета 68 (100.0%), самая длинная серия 68 подряд
  режим вкл (повтор 2):
    N=0     отрисовок 0     GPU 2.779  CPU 0.112 мс; доставлено 135 кадров за 1501 мс = 89.9 к/с из 90 (100%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 13.067  CPU 2.634 мс; доставлено 115 кадров за 1511 мс = 76.1 к/с из 90 (85%); сверх бюджета 115 (100.0%), самая длинная серия 115 подряд
  режим выкл (повтор 2):
    N=0     отрисовок 0     GPU 2.651  CPU 0.112 мс; доставлено 136 кадров за 1510 мс = 90.1 к/с из 90 (100%); промахов по бюджету нет
    N=1600  отрисовок 1600  GPU 8.943  CPU 1.941 мс; доставлено 135 кадров за 1500 мс = 90.0 к/с из 90 (100%); промахов по бюджету нет
  PASS  S гейт состояния: enabled и relax вставали в каждый режим
  PASS  S гейт счётчика: все точки дали ровно N отрисовок
  PASS  S контроль порядка: «выкл» в начале и в конце на N=1600 расходятся на 0.1% — в пределах дрейфа 15%
  ...
  PASS  S цена на кадр приложения (GPU против «выкл», порог — два разброса «выкл»): N=0 вкл -0.140 мс в шуме; N=0 вкл+relax +0.708 мс в шуме; N=1600 вкл +4.118 мс ЗНАЧИМО; N=1600 вкл+relax +8.067 мс ЗНАЧИМО

S визуально: 30 с, полоса движется, синтез меняется каждые 5 с. ЗЕЛЁНАЯ — синтез ВКЛ, КРАСНАЯ — ВЫКЛ
    0 с: синтез ВКЛ (зелёная)
    5 с: синтез ВЫКЛ (красная)
    10 с: синтез ВКЛ (зелёная)
    15 с: синтез ВЫКЛ (красная)
    20 с: синтез ВКЛ (зелёная)
    25 с: синтез ВЫКЛ (красная)
