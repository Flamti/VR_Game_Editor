...
- Описание кремния XR2 Gen 2 (2 больших + 4 малых ядра, TDP 4–6 Вт) — верно. Но вывод «CPU
  становится главным узким местом», который документ из него делает, **опровергнут измерением
  для отрисовки** — см. раздел выше. Для игровой логики и физики он может быть верен: это не
  проверялось.
...
Документ считает бюджет от 90 Гц (11.1 мс) и 72 Гц (13.8 мс). Измерено: устройство предлагает
**[72, 80, 90, 120] Гц**, но Godot по умолчанию работает на **72.0 Гц** и выше не запрашивает,
хотя `XR_FB_display_refresh_rate` доступен. То есть бюджет документа достижим, но **сейчас
проект живёт в 13.89 мс**, а не в 11.1 — и это управляемый параметр.

Термальный троттлинг, которого документ ожидал «после нескольких минут»: за 18 минут при 82%
бюджета по GPU **устойчивой деградации нет**. Есть разогрев ~6% за первые семь минут с
последующей стабилизацией — выход на режим, а не троттлинг.

Оговорка о границе: все прогоны шли на 72 Гц — самой низкой доступной частоте, где ОС физически
не может сбросить ниже. На 90 Гц запас впятеро меньше, и ожидание документа может подтвердиться.
Не проверялось.
...
## Появившиеся источники и что они изменили (2026-09-13)

Каталог внешних источников с оценкой по уровням доверия — **`docs/research/sources.md`**. Этот
файл разбирает один документ, тот — все остальные.

Существенное для этого разбора: **HorizonOS 2.7 снял потолок в 120 Гц на Quest 3.** По
документации Meta устройство даёт 72–207 Гц без какой-либо настройки, а перечисление частот через
OpenXR неисчерпывающе по замыслу. Прибор это подтвердил с той стороны, с какой мог: 144 Гц
работают, доставка 144.1 к/с.

Для разбора исследования это означает, что его расчёты от 90 Гц устарели не в ту сторону, в
какую казалось: потолок не 90 и не 120, а 207 — и тем острее вопрос, какую частоту проект
выбирает целью (ADR-0006 и его поправка).
