...
## Гейт контекста щипка: средний и безымянный вытянуты.
##
## Свободная рука сама замыкает щипок (окно тычка правой, прогоны 14–15): большой
## у указательного до 3–9 мм, порогом расстояния не отделить — это настоящая поза
## щипка. Отделил сгиб ОСТАЛЬНЫХ пальцев, max(средний, безымянный) по кадрам:
## щипки по сигналу p99 0.038, максимум 0.041 (4156 кадров); свободная рука
## минимум 0.085, p5 0.139 (514 кадров). Порог — середина зазора.
##
## Прогон 21 (R v4) проверил риск расслабленного щипка: 4 из 4, но сгиб остальных
## доходил до 0.054 при прежнем пороге 0.063 — запас 0.009. Свободная рука в том же
## прогоне — минимум 0.092 (гейт снял 2 её щипка). Порог сдвинут на середину зазора
## 0.054…0.085 решением владельца. Граница применимости — один человек, три сессии.
const PINCH_OTHERS_MAX := 0.070
...
static func others_curl(d: Dictionary) -> float:
	return maxf(d["curl_middle"], d["curl_ring"])
...
## gated=false — классификатор v3 без гейта контекста, для сравнения в R v4.
## Гейт проверяется только на ВХОДЕ: удерживаемый щипок им не обрывается.
static func classify(d: Dictionary, prev: String, gated: bool = true) -> String:
	...
	if mc > (FIST_EXIT if prev == "fist" else FIST_ENTER):
		return "fist"
	if prev == "pinch":
		return "pinch" if d["pinch_m"] < PINCH_EXIT else ""
	if d["pinch_m"] < PINCH_ENTER and (not gated or others_curl(d) < PINCH_OTHERS_MAX):
		return "pinch"
	return ""
