...
## Повесить контейнер нагрузки на камеру, чтобы число отрисовок не зависело от
## того, куда смотрит шлем.
##
## Разбор TSV длинных прогонов 2026-09-12 показал, чем кончается нагрузка в
## мире: 11 пустых и 59 частичных выборок из 1078 во втором прогоне — от 60 до
## 1597 вызовов при заявленных 1600, и GPU-время падает вместе со счётом (604
## вызова → 4.48 мс, 0 → 2.608 мс, ровно пустая сцена). Узлы в фазе E не
## создаются и не удаляются; частичный счёт объясняет только отсечение по
## пирамиде при повороте головы. Частичные выборки попадали в четверти дрейфа.
## Та же причина — вероятный корень «N=800 → 658» в свипе прогона 1.
##
## Фазы L и L4 крепились к камере с самого начала (обкатка 2026-09-13).
## Фаза P осталась в мире: её покрытие считается проекцией в мировых осях.
static func attach_load(host: Node, parent: Node, container: Node3D) -> void:
	container.position = Vector3(0, 0, -2)
	var cam := host.get_viewport().get_camera_3d()
	if cam != null:
		cam.add_child(container)
	else:
