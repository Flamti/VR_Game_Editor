...
##   - режим игровой зоны — roomscale (Local Floor: пол шлема, начало — где человек стоит);
...
## Зона, у которой есть ПОЛ: без неё высота головы отсчитывается не от земли. Возвращает режим,
## который получился.
##
## Зона с полом уже есть — НЕ переключаем: смена пространства сдвигает позу камеры разом, и после
## посадки это рывок вида. Нет — просим roomscale (Local Floor), и только его. Прежде первой шла
## проба stage: Quest без границы его не даёт, а Meta не рекомендует Stage ни для каких приложений
## (решение владельца 2026-09-26; пространство Local Floor задано и в project.godot).
##
...
func ensure_floor() -> int:
	...
	if has_floor(int(play_area_mode.call())):
		return int(play_area_mode.call())
	set_play_area.call(XRInterface.XR_PLAY_AREA_ROOMSCALE)
	return int(play_area_mode.call())
...
func reset(on_measure: Callable = Callable()) -> String:
	...
	if not falsify_no_reset:
		staged = bool(set_play_area.call(XRInterface.XR_PLAY_AREA_ROOMSCALE))
		recenter.call()
	var mode_after: int = play_area_mode.call()
	if on_measure.is_valid():
		on_measure.call()
	last_detail = "режим зоны %d → %d (roomscale %s), положение сброшено %s, высота глаз измеряется" % [
